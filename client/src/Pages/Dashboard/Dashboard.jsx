            ...
            <div className='flex flex-col w-full gap-2 mt-5'>
              {
                role === 'Admin' ?
                  <>
                    <span className={` ${ActivePath.includes('dashboard') ? "bg-slate-400 text-slate-100" : " bg-slate-50 text-black"} rounded-lg cursor-pointer border py-2 flex gap-2 items-center w-full sm:pl-5 pl-[6px] sm:pr-12 sm:w-full pr-1`} onClick={() => { navigate('') }}>
                      <IoHomeOutline />{sideBarToggle ? <span className='m-0 p-0'>NPOS</span> : ''}
                    </span>
                    <span className={` ${pathName.includes('reports') ? "bg-slate-400 text-slate-100" : " bg-slate-50 text-black"} cursor-pointer border rounded-lg py-2 flex gap-2 sm:w-full w-full  items-center pl-[6px] sm:pr-12 sm:pl-4 pr-0`} onClick={() => { navigate('reports') }}>
                      <LiaClipboardListSolid />{sideBarToggle ? <span className='m-0 p-0'>Reports</span> : ''}
                    </span>
                  </>
                  :
                  <>
                    <span className={` ${ActivePath.includes('dashboard') ? "bg-slate-400 text-slate-100" : " bg-slate-50 text-black"} cursor-pointer border rounded-lg py-2 flex gap-2 sm:w-full  items-center pl-[4.8px] sm:pr-12 sm:pl-4 pr-[4.5px]`} onClick={() => { navigate('') }}>
